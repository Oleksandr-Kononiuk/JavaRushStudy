...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        Set<Integer> deepFriends = new HashSet<>(0);
        Set<Integer> allFriendsAndPotentialFriends = getAllFriends(index);

        //"deep" times we looking up all the potential friends
        for (int d = 0; d < deep; d++) {
            allFriendsAndPotentialFriends.addAll(deepFriends);
            deepFriends.clear();

            for (Integer j : allFriendsAndPotentialFriends) {
                deepFriends.addAll(getAllFriends(j));
            }
        }
        //removing our "index"
    ...
    //Looking up all the "index" friends
    ...
    private static boolean[][] generateRelationships() {
        return new boolean[][]{
                ...
                //0 5 is friends but 1 5 not friends
        };
    }
}